...
// NOTE: We are now using path.join(__dirname, 'seeds', '...') to ensure the CSV paths
// are resolved relative to the seed.ts file's directory (db/), making the script
// independent of the Current Working Directory (CWD).

// 1. FIX: Define __filename and __dirname for ES Module environments (like when using tsx)
...
async function seed() {
  ...
  await client.delete(profile).catch(() => {});
  ...
  // 2. PATH FIX: Use path.join(__dirname, 'seeds', 'devices.csv')
  const devices = await csvtojson().fromFile(path.join(__dirname, "seeds", "devices.csv")); 
  let i = 0;
  ...
  // 3. PATH FIX: Use path.join(__dirname, 'seeds', 'sensors.csv')
  const sensors = await csvtojson().fromFile(path.join(__dirname, "seeds", "sensors.csv"));
  ...
  try {
    // 4. PATH FIX: Use path.join(__dirname, 'seeds', 'measurements.csv')
    const measurements = await csvtojson().fromFile(
      path.join(__dirname, "seeds", "measurements.csv"),
    );
    let k = 0;
